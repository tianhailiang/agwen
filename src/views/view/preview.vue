<template>
  <div class="view-layout">
    ...
    <div class="main" v-if="(!isExpired && !isNotPublish) || isAdmin">
      <div class="header">
        <h1>{{gettersNaire.title}}</h1>
      </div>
      <div class="content">
        <div class="intro">
          <p>{{gettersNaire.intro}}</p>
          <p>截止日期：{{gettersNaire.deadline | timeFormat}}</p>
        </div>
        <div class="user-info">
        ...
        <questionList :question-list="gettersNaire.topic" :isPreview="true">
  ...
  export default {
    ...
    methods: {
      ...
      validateNaire () {
        ...
        this.gettersNaire.topic.forEach((item, index) => {
          if (item.isRequired) {
            if (item.type === '文本') {
      ...
      submitNaire () {
        ...
        const nId = this.gettersNaire.n_id
        const result = []
        this.gettersNaire.topic.forEach((question, index) => {
          if (question.type === '单选') {
            const curQues = {
              n_id: nId,
              u_id: this.userId,
    ...
    computed: {
      ...
      isExpired () {
        return Number(this.gettersNaire.deadline) < new Date().getTime()
      },
      isNotPublish () {
        return this.gettersNaire.status === '0'
      },
      gettersNaire () {
